...
<table class="table table-striped mt-5 pt-5">
    ...
    while ($row = $stmt->fetch()) {
        ...
        $u_stmt->execute(['u_id' => $currUserId]);
        if ($u_row = $u_stmt->fetch()) {
            $userFullname = $u_row['u_name'] . " " . $u_row['u_lastName'];
            if (empty(trim($userFullname))) {
                $msg = "<div class='alert alert-info mt-5 text-center'>
                            <i class='fa fa-info-circle'></i> Please complete your profile 
                            <a class='btn btn-link' href='edit_profile.php'>Edit Profile</a>
                        </div>";
            }
        }
